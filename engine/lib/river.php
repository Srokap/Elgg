...
function elgg_create_river_item(array $options = array()) {
	...
	$type = sanitise_string($values['type']);
	$subtype = sanitise_string($values['subtype']);
	$action_type = sanitise_string($values['action_type']);
	$access_id = sanitise_int($values['access_id']);
	$view = sanitise_string($values['view']);
